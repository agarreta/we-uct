...
class WordEquation(object):
    ...
    def get_string_form(self):
        if self.args.use_length_constraints:
            ...
            for i,y in enumerate(self.coefficients_variables_lc):
                for _ in range(len(self.coefficients_variables_lc)):
                    for x in self.args.VARIABLES:
                        if y[_][x] > 0:
                            lc_string += str(y[_][x]) + x
    ...
    def nullify_lc(self, num,i):
        self.coefficients_variables_lc[i][num] = {x: 0 for x in self.args.VARIABLES}
        self.ell[i][num] = 0
