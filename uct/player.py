...
class Player(object):
    ...
    def execute_episode(self, eq=None, verbose=0):
        ...
        while True:
            ...
            if eq.w in self.mcts.final_state_value:
                ...
                eq = self.we.utils.check_satisfiability(eq, smt_time=self.args.mcts_smt_time_max)
                self.mcts.final_state_value[eq.w]=eq.sat
    ...
    def play(self, level_list=None):
        ...
        if self.mode == 'test' and 'pool' in self.args.pool_name_load:
            ...
            if os.path.exists(file_name):
                ...
                max_level = 25
                npool = []
                level_slots = [range(math.floor(10 + 1.6 * i), math.floor(10 + 1.6 * (i + 1))) for i in range(0, 9)] if '20_5_3' in self.args.pool_name_load else [range(math.floor(3 + 1. * i), math.floor(3 + 1. * (i + 1))) for i in range(0, 9)]
